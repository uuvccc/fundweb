...
Write-Host "基金监控系统 - 修复版启动脚本" -ForegroundColor Cyan
Write-Host "========================================" -ForegroundColor Cyan
...
if (-not $SkipDeps) {
    ...
    try {
        ...
        Write-Host "⚠️  pip升级失败，继续执行..." -ForegroundColor Yellow
    }

    Write-Host "安装Python依赖包..." -ForegroundColor Yellow
    try {
        if ($ForceReinstall) {
            Write-Host "强制重新安装依赖..." -ForegroundColor Yellow
        ...
        Write-Host "尝试使用兼容模式安装..." -ForegroundColor Yellow
        try {
            pip install --no-deps -r requirements.txt
            Write-Host "✅ 依赖安装完成（兼容模式）" -ForegroundColor Green
        } catch {
            Write-Host "❌ 依赖安装完全失败" -ForegroundColor Red
            Read-Host "按回车键退出"
            exit 1
        }
    }
...
try {
    python -c "import flask_sqlalchemy; print('Flask-SQLAlchemy:', flask_sqlalchemy.__version__)"
    python -c "import flask; print('Flask:', flask.__version__)"
    python -c "import requests; print('Requests:', requests.__version__)"
    Write-Host "✅ 关键依赖验证通过" -ForegroundColor Green
} catch {
    Write-Host "❌ 关键依赖验证失败" -ForegroundColor Red
    Write-Host "请尝试重新安装依赖或使用 --ForceReinstall 参数" -ForegroundColor Yellow
    Read-Host "按回车键退出"
    exit 1
}
...
try {
    ...
    Write-Host "4. 尝试使用 --ForceReinstall 参数重新安装依赖" -ForegroundColor Yellow
    Read-Host "按回车键退出"
    exit 1
}
